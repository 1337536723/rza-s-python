										...
										if d(text="Import from contacts").wait.exists():
											...
											if d(text="Contacts").wait.exists():
												...
												if d(index=2,className="android.view.ViewGroup").wait.exists():
													...
													c.common.log("index=2未找到，保存截图")
													c.common.save_fail_img()
											else:
												c.common.log("text=Contacts未找到，保存截图")
												c.common.save_fail_img()
										else:
											c.common.log("text=Import from contacts未找到，保存截图")
											c.common.save_fail_img()
									else:
										c.common.log("description=More options未找到，保存截图")
										c.common.save_fail_img()
								else:
									c.common.log("text=Add contact未找到，保存截图")
									c.common.save_fail_img()
							else:
								c.common.log("description=More options未找到，保存截图")
								c.common.save_fail_img()
						else:
							c.common.log("text=FDN list未找到，保存截图")
							c.common.save_fail_img()
					else:
						c.common.log("text=Cancel未找到，保存截图")
						c.common.save_fail_img()								
				else:
					c.common.log("text=Change PIN2未找到，保存截图")
					c.common.save_fail_img()									
			else:
				c.common.log("text=Cancel未找到，保存截图")
				c.common.save_fail_img()																						
		else:
			c.common.log("text=Enable FDN未找到，保存截图")
			c.common.save_fail_img()
	else:
		c.common.log("text=Fixed Dialing Numbers未找到，保存截图")
		c.common.save_fail_img()
			...
			if d(text="Call forwarding settings").wait.exists(timeout=29000):
				...
				for i in range(4):
					if d(text=options1[i],resourceId="android:id/title").wait.exists():
						...
						c.common.log("text="+options1[i]+"未找到，保存截图")
						c.common.save_fail_img()
				time.sleep(1)		
				d.press("back")
							...
							if d(text="Outgoing call barring").wait.exists(timeout=29000):
								...
								for j in range(4):
									if d(text=options2[j]).wait.exists():
										...
										c.common.log("text="+options2[j]+"未找到，保存截图")
										c.common.save_fail_img()
								time.sleep(1)
								d.press("back")
								time.sleep(1)
								...
								c.common.log("text=Outgoing call barring未找到，保存截图")
								c.common.save_fail_img()
						else:
							c.common.log("text=Call barring未找到，保存截图")
							c.common.save_fail_img()
					else:
						c.common.log("text=Call waiting未找到，保存截图")
						c.common.save_fail_img()
				else:
					c.common.log("text=Additional settings未找到，保存截图")
					c.common.save_fail_img()
			else:
				c.common.log("text=Call forwarding settings未找到，保存截图")
				c.common.save_fail_img()
		else:
			c.common.log("text=Call forwarding未找到，保存截图")
			c.common.save_fail_img()
	else:
		c.common.log("text=GSM call settings未找到，保存截图")
		c.common.save_fail_img()	
...
def loop_dmnr():
	if d(text="Dual mic noise reduction").wait.exists():
		...
		c.common.log("text=Dual mic noise reduction未找到，保存截图")
		c.common.save_fail_img()
